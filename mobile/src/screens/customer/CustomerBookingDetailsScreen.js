            ...
            >
                ...
                <View style={styles.section}>
                    ...
                    {(() => {
                        ...
                        let slots = [];
                        try {
                            dates = typeof booking.job_date === 'string' && booking.job_date.startsWith('[') ? JSON.parse(booking.job_date) : booking.job_date;
                            if (typeof dates === 'string') dates = dates.split(',').map(s => s.trim()).filter(Boolean);
                            if (!Array.isArray(dates)) dates = [dates];

                            slots = typeof booking.job_time_slot === 'string' && booking.job_time_slot.startsWith('[') ? JSON.parse(booking.job_time_slot) : booking.job_time_slot;
                            if (typeof slots === 'string') slots = slots.split(',').map(s => s.trim()).filter(Boolean);
                            if (!Array.isArray(slots)) slots = [slots];
                        } catch (e) {
                            dates = [booking.job_date];
                            slots = [booking.job_time_slot];
                        }
