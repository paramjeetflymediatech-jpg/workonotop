...
//       const data = await res.json()
//       if (data.success) {
//         showMessage('success', 'Provider status updated')
//         loadAllUsers()
//       } else {
...
//           Manage all customers, admins, and providers
//         </p>
//       </div>
...
//         />
//         <svg className={`absolute left-3 top-3.5 w-4 h-4 ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}
...
//                   <tr key={c.id}
//                     className={`cursor-pointer transition-colors ${isDarkMode ? 'hover:bg-slate-800' : 'hover:bg-gray-50'}`}
...
//                         : <span className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>No</span>}
//                     </td>
...
//                   {[1,2,3,4,5].map(star => (
//                     <svg key={star} className={`w-3.5 h-3.5 ${star <= getRatingValue(p.rating) ? 'text-yellow-400' : isDarkMode ? 'text-slate-600' : 'text-gray-200'}`}
...
//                               <span className={`px-2 py-0.5 text-xs rounded-full ${getStatusColor(b.status)}`}>
//                                 {b.status?.replace('_', ' ')}
//                               </span>
//                             </td>
...
export default function Users() {
  ...
  // New state for provider details
  ...
  // New function to load provider details and jobs
  const loadProviderDetails = async (provider) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const stats = calculateProviderStats(jobs, provider)
  ...
  const calculateProviderStats = (jobs, provider) => {
    ...
    const totalEarnings = completed.reduce((sum, job) => {
      return sum + parseFloat(job.final_provider_amount || job.provider_amount || 0)
    }, 0)
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {activeTab === 'providers' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredProviders.length > 0 ? filteredProviders.map(p => (
            <div key={p.id} className={`${card} p-5 cursor-pointer hover:shadow-xl transition-all`}
              ...
              <select
                ...
                onChange={e => {
                  e.stopPropagation();
                  updateProviderStatus(p.id, e.target.value);
                }}
          ...
          <div className={`relative rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${getStatusColor(selectedProvider.status)}`}>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                      <p className={subText}>Total Jobs</p>
                      <p className={`text-2xl font-bold mt-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                        {providerStats.totalJobs}
                      </p>
                    </div>
                    <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                      <p className={subText}>Completed</p>
                      <p className="text-2xl font-bold mt-1 text-green-500">{providerStats.completedJobs}</p>
                    </div>
                    <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                      <p className={subText}>Pending</p>
                      <p className="text-2xl font-bold mt-1 text-yellow-500">{providerStats.pendingJobs}</p>
                    </div>
                    <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                      <p className={subText}>Completion Rate</p>
                      <p className="text-2xl font-bold mt-1 text-teal-500">{providerStats.completionRate}%</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                      ...
                      <p className="text-2xl font-bold mt-1 text-blue-500">
                        {providerStats.totalHoursWorked} hrs
                      </p>
                    </div>
                    <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <div className="flex items-center gap-0.5">
                          {[1,2,3,4,5].map(star => (
                            <svg key={star} className={`w-4 h-4 ${star <= getRatingValue(providerStats.avgRating) ? 'text-yellow-400' : isDarkMode ? 'text-slate-600' : 'text-gray-200'}`}
                              fill="currentColor" viewBox="0 0 20 20">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                          ))}
                          ...
                          <div key={index} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 h-6 bg-gray-200 dark:bg-slate-700 rounded-full overflow-hidden">
                              <div 
                                ...
                                style={{ 
                                  width: `${Math.min(100, (month.earnings / (providerStats.totalEarnings / 6) * 100))}%` 
                                }}
                              />
                            </div>
                  ...
                  <div className={`border-t pt-5 ${isDarkMode ? 'border-slate-700' : 'border-gray-200'}`}>
                    <h5 className={`font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                      Job History ({providerJobs.length})
                    </h5>
                    ...
                    {providerJobs.length > 0 ? (
                      ...
                      <p className={`text-sm text-center py-6 ${isDarkMode ? 'text-slate-400' : 'text-gray-400'}`}>
            ...
            <div className={`p-5 border-t flex justify-end gap-3 ${isDarkMode ? 'border-slate-700' : 'border-gray-200'}`}>
              <select
                ...
                onChange={(e) => {
                  updateProviderStatus(selectedProvider.id, e.target.value);
                  setSelectedProvider({...selectedProvider, status: e.target.value});
                }}
              ...
              <button
                onClick={() => setIsProviderDetailsOpen(false)}
                className={`px-4 py-2 rounded-lg text-sm font-medium ${isDarkMode ? 'bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                Close
              </button>
