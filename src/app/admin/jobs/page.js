...
export default function JobRequests() {
  ...
  // Show only base price for initial amount
  const calculateInitialAmount = (job) => {
    return parseFloat(job.service_price).toFixed(2)
  }

  // Check if job has overtime rate set
  const hasOvertimeRate = (job) => {
    return parseFloat(job.additional_price || 0) > 0
  }

  // Get duration from service_duration (from services table)
  const getDuration = (job) => {
    return job.service_duration || 60 // Default to 60 if not set
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredJobs.length > 0 ? (
          filteredJobs.map((job) => {
            ...
            const hasOvertime = hasOvertimeRate(job)
              ...
              >
                ...
                >
                  ...
                  <div className="col-span-2 mt-1">
                    <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-gray-500'}`}>Duration</p>
                    <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                ...
                {/* Provider Assignment & Pricing Info */}
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-gray-500'}`}>Initial Amount</p>
                    <p className={`text-lg font-bold text-teal-600`}>
                      ${calculateInitialAmount(job)}
                    </p>
                    {/* Show overtime indicator if applicable */}
                    {hasOvertime && (
                      <p className={`text-[10px] ${isDarkMode ? 'text-slate-500' : 'text-gray-500'}`}>
                        +${parseFloat(job.additional_price).toFixed(2)}/hr overtime
                      </p>
                    )}
                  </div>
                </div>
                    ...
                    <button
                      onClick={() => {
                        if (tradespeople.length > 0) {
                          assignProvider(job.id, tradespeople[0].id)
                        } else {
                          showMessage('error', 'No providers available')
                        }
                      }}
