...
export default function StatesPage() {
  ...
  useEffect(() => {
    fetchStates(page, debouncedSearch)
  }, [page, debouncedSearch])

  const fetchStates = async (currentPage, searchQuery = '') => {
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/states?page=${currentPage}&limit=10&search=${encodeURIComponent(searchQuery)}`)
  ...
  const handleToggleStatus = async (state) => {
    const action = state.is_active === 1 ? 'deactivate' : 'activate'
    const newStatus = state.is_active === 1 ? 0 : 1
    
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: `Do you want to ${action} ${state.name}?`,
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        const res = await fetch(`/api/admin/states/${state.id}`, {
          ...
          body: JSON.stringify({ name: state.name, is_active: newStatus })
        })
        const data = await res.json()
        if (data.success) {
          toast.success(data.message)
        ...
        toast.error(`Failed to ${action} state`)
      }
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">States</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Manage states for service areas.</p>
        </div>
        <Link
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
              {loading ? (
                ...
                states.map((state) => (
                  <tr key={state.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition">
