...
export default function Step3Stripe({ initialData, onNext, onBack, providerId, providerEmail }) {
  ...
  const handleSkip = () => {
    setSkipMode(true);
    // Mark Stripe as not completed but proceed to next step
    setTimeout(() => {
      onNext({ stripeOnboardingComplete: false, skipped: true });
    }, 500);
  };

  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Set Up Payments (Optional)</h2>
      
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
        <p className="text-sm text-amber-800">
          <strong>✨ Stripe is optional for now!</strong> You can connect your bank account later from your profile. 
          Without Stripe, you won't receive payouts for completed jobs.
        </p>
      </div>
      ...
      {skipMode ? (
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Stripe Skipped</h3>
          <p className="text-gray-600 mb-4">You can connect Stripe later from your profile settings.</p>
          <p className="text-sm text-gray-500">Redirecting to review page...</p>
        </div>
      ) : stripeStatus === 'completed' ? (
        <div className="text-center py-8">
        ...
        <div className="border-2 border-gray-200 rounded-lg p-6">
          ...
          <p className="text-xs text-gray-500 text-center mt-4">
            You can always connect Stripe later from your profile settings.
          </p>
        </div>
      )}
      ...
      <div className="flex justify-between pt-6">
        <button
          onClick={onBack}
          disabled={stripeStatus === 'completed' || skipMode}
          className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Back
        </button>
