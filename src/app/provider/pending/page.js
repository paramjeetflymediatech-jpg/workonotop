...
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { Clock, CheckCircle, RefreshCw, Mail } from 'lucide-react';
...
export default function ProviderPending() {
  ...
  const steps = [
    ...
    { label: 'Payment Connected', desc: 'Stripe account verified', done: true },
                ...
                {steps.map((s, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                      s.done ? 'bg-green-100' : s.current ? 'bg-amber-100' : 'bg-gray-100'
                    }`}>
                      {s.done ? (
                    ...
                    <div>
                      <p className={`text-sm font-medium ${s.current ? 'text-amber-800' : s.done ? 'text-gray-800' : 'text-gray-400'}`}>{s.label}</p>
