...
function ConfirmModal({ isOpen, onClose, onConfirm, title, message, amount, duration, overtime, isProrated, isOvertime, standardMinutes, actualMinutes, basePrice }) {
  ...
  // Local helper to format minutes into readable hours/minutes
          ...
          <div className="bg-gray-50 rounded-xl p-3 mb-3 text-left">
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatDurationLocal(standardMinutes)}</span>
              </div>
              <div className="flex justify-between">
...
export default function BookingDetails() {
  ...
  // Determine standard minutes robustly: prefer explicit minute fields from booking or service
  const standardMinutes = parseInt(
    booking.standard_duration_minutes ?? booking.duration_minutes ?? booking.service?.duration_minutes ?? booking.service?.duration ?? 60,
    10
  ) || 60
  const basePrice = booking.pricing?.base_price || 0
  const overtimeRate = booking.pricing?.overtime_rate || 0
              ...
              <div className={`rounded-xl p-4 mb-5 ${
                ...
                <div className="mt-3 text-xs border-t pt-2 border-gray-200">
                  <div className="flex justify-between text-gray-600">
                    <span>Base price (standard: {formatDuration(standardMinutes)}):</span>
                ...
                <p className="text-xs text-gray-500 mt-2">
                  {isProrated 
                    ? `⏱️ You're only paying for ${actualMinutes}min worked (${Math.round((actualMinutes/standardMinutes)*100)}% of standard time)`
                    : isOvertime
                    ? `⏰ Includes ${overtimeMinutes}min overtime at ${formatCurrency(overtimeRate)}/hour`
                    : '✅ Payment was held securely. Final amount matches standard.'
                  }
                </p>
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Base Price (standard: {formatDuration(standardMinutes)})</span>
                  ...
                  {booking.status === 'awaiting_approval' && (
                    <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-xs text-blue-700">
                        ...
                        {isProrated 
                          ? ` Since job took ${actualMinutes}min (less than standard), you pay ${Math.round((actualMinutes/standardMinutes)*100)}% of standard time.`
                          : isOvertime
