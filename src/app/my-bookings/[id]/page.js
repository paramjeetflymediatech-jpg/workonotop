...
//               className="w-full border rounded-xl p-3 text-sm outline-none focus:border-teal-500 resize-none"
//               maxLength={500}
//             />
...
//               className="flex-1 py-3 bg-teal-600 text-white rounded-xl font-bold text-sm disabled:opacity-50 hover:bg-teal-700"
//             >
...
//           checkExistingReview(bookingData.id)
//         }
//       } else {
...
//   const checkExistingReview = async (bookingId) => {
//     try {
//       const res = await fetch(`/api/reviews?booking_id=${bookingId}`)
//       const data = await res.json()
//       if (data.success && data.data.length > 0) {
//         setHasReviewed(true)
//       } else {
...
//             <Link href="/my-bookings" className="inline-flex items-center gap-2 bg-teal-600 text-white px-6 py-3 rounded-xl">
...
//                       <p className="text-sm text-gray-500">Share your feedback to help other customers</p>
//                     </div>
//                   </div>
//                   <button
//                     onClick={() => setShowReviewModal(true)}
//                     className="w-full bg-teal-600 hover:bg-teal-700 text-white py-4 rounded-xl font-bold"
//                   >
//                     Write a Review
//                   </button>
//                 </div>
...
//                     <User className="w-5 h-5 text-teal-600" /> Provider
//                   </h2>
...
//                                 star <= Math.round(booking.provider_rating)
//                                   ? 'fill-yellow-400 text-yellow-400'
//                                   : 'text-gray-300'
//                               }`} />
//                             ))}
...
//                             {parseFloat(booking.provider_rating).toFixed(1)} ({booking.provider_reviews || 0} reviews)
//                           </span>
//                         </div>
//                       )}
//                     </div>
...
//                 <div className="bg-white rounded-2xl p-6 border">
//                   <h2 className="text-base font-bold mb-4 flex items-center gap-2">
//                     <MapPin className="w-5 h-5 text-teal-600" /> Service Location
//                   </h2>
//                   <div className="space-y-2">
//                     <p className="text-gray-800">
//                       {booking.address_line1}
//                       {booking.address_line2 && <span>, {booking.address_line2}</span>}
//                     </p>
//                     <p className="text-gray-600">
//                       {booking.city}
//                       {booking.postal_code && <span>, {booking.postal_code}</span>}
//                     </p>
                    
//                     {/* Additional location details if available */}
//                     {(booking.parking_access || booking.elevator_access || booking.has_pets) && (
//                       <div className="mt-3 pt-3 border-t">
//                         <p className="text-sm font-medium text-gray-700 mb-2">Location Details:</p>
...
//                             <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-purple-50 text-purple-700">
//                               🛗 Elevator Access
//                             </span>
//                           )}
//                           {booking.has_pets === 1 && (
//                             <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-amber-50 text-amber-700">
//                               🐕 Pets on Premises
//                             </span>
//                           )}
//                         </div>
...
//                       <span className="font-medium">{fmt(booking.authorized_amount)}</span>
//                     </div>
//                   )}
...
//                   {isOvertime && (
//                     <div className="flex justify-between text-sm">
//                       <span className="text-gray-600">Overtime ({overtimeMins}min):</span>
...
//                       <span className={`font-bold text-lg ${isOvertime ? 'text-purple-600' : 'text-teal-600'}`}>
//                         {fmt(customerTotal)}
//                       </span>
//                     </div>
//                   </div>
...
//                 <div className="bg-white rounded-2xl p-6 border">
//                   <h2 className="text-base font-bold mb-4 flex items-center gap-2">
//                     <Clock className="w-5 h-5 text-teal-600" /> Status Timeline
//                   </h2>
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 py-6 md:py-8">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <div className="flex gap-2 mb-6 border-b">
            {tabs.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium border-b-2 capitalize transition ${activeTab === tab ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                {tab}{tab === 'photos' && <span className="ml-1 text-xs bg-gray-100 text-gray-500 rounded-full px-1.5">{allPhotos.length}</span>}
              </button>
            ))}
          </div>
          ...
          {activeTab === 'details' && (
            <div className="space-y-5">
              ...
              {/* ── PAYMENT DETAILS — invoice style matching success page ── */}
              <div className="bg-white rounded-2xl p-6 border">
                <h2 className="text-base font-bold mb-4 flex items-center gap-2">
                  <CreditCard className="w-5 h-5 text-teal-600" /> Payment Details
                </h2>
                  ...
                  <div className="px-4 py-3 space-y-0.5">
                    ...
                    <div className="flex justify-between items-center py-2.5 border-b border-gray-200">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">Flat rate ({standardMinutes}min)</p>
                      </div>
                      <span className="text-sm font-bold text-gray-900">{fmt(basePrice)}</span>
                    </div>

                    {/* Overtime hold (always show if overtime rate exists) */}
                    {overtimeRate > 0 && (
                      <div className="flex justify-between items-center py-2.5 border-b border-gray-200">
                        <div className="flex-1 pr-4">
                          ...
                          <p className="text-xs text-gray-400">{fmt(overtimeRate)}/hr × 2 hrs (max hold)</p>
                        </div>
                        <span className="text-sm font-bold text-amber-600">+{fmt(overtimeHoldAmount)}</span>
                      </div>
                    )}

                    {/* Actual overtime (after job done) */}
                    {isOvertime && (
                      <div className="flex justify-between items-center py-2.5 border-b border-gray-200">
                        <div className="flex-1 pr-4">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            <Clock className="w-3.5 h-3.5 text-purple-500" />
                            <p className="text-sm font-medium text-purple-700">Actual Overtime Used</p>
                          </div>
                    ...
                    <div className="flex justify-between items-center py-2.5 border-b border-gray-200">
                      <div>
                        <p className="text-sm font-medium text-gray-600">Total Authorized (Hold)</p>
                    ...
                    {/* You Pay — big prominent */}
                    <div className="flex justify-between items-center pt-3 pb-1">
                      <div>
                        <p className="text-sm font-bold text-gray-900">
                          {booking.status === 'completed' ? 'Amount Charged' : 'You Pay'}
                        </p>
                        <p className="text-xs text-gray-400">
                          {booking.status === 'completed'
                            ? 'Final amount after job completion'
                            : 'Base + actual overtime used'}
                        </p>
                      </div>
                      <span className={`text-2xl font-extrabold ${isOvertime ? 'text-purple-700' : 'text-teal-600'}`}>
                        {fmt(customerTotal)}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="bg-amber-50 border-t border-amber-100 px-4 py-3 flex gap-3 items-start">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                          Your card is authorized for <span className="font-bold">{fmt(totalAuthorized)}</span>.{' '}
                          You only pay <span className="font-bold">{fmt(basePrice)}</span> (base) + actual overtime used.
          ...
          {activeTab === 'photos' && (
            <div className="space-y-5">
              ...
              {allPhotos.length === 0 && (
                <div className="bg-white rounded-2xl p-12 border text-center">
                  <ImageIcon className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-400 text-sm">No photos yet</p>
                </div>
              )}
            </div>
          )}
