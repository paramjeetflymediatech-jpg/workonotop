...
export default function SkillForm({ isEdit = false, initialData = null }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(isEdit ? 'Failed to update skill' : 'Failed to save skill');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
          ...
          >
            {submitting ? 'Saving...' : (isEdit ? 'Update Skill' : 'Add Skill')}
          </button>
        </div>
      </form>
    </div>
