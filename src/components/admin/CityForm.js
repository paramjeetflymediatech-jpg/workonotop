'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import toast from 'react-hot-toast';

export default function CityForm({ isEdit = false, initialData = null }) {
  const router = useRouter();
  const [submitting, setSubmitting] = useState(false);
  
  const [formData, setFormData] = useState({
    name: '',
    cluster_group: '',
    is_active: true,
    citiesInput: ''
  });

  useEffect(() => {
    if (initialData) {
      setFormData({
        name: initialData.name || '',
        cluster_group: initialData.cluster_group || '',
        is_active: initialData.is_active !== 0,
        citiesInput: Array.isArray(initialData.cities) ? initialData.cities.join(', ') : ''
      });
    }
  }, [initialData]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.cluster_group) {
      toast.error('Name and Cluster Group are required');
      return;
    }

    setSubmitting(true);
    try {
      const url = isEdit && initialData 
        ? `/api/admin/service-areas/${initialData.id}`
        : '/api/admin/service-areas';
      const method = isEdit ? 'PUT' : 'POST';

      const citiesArray = formData.citiesInput 
        ? formData.citiesInput.split(',').map(c => c.trim()).filter(c => c.length > 0)
        : [];

      const payload = {
        name: formData.name,
        cluster_group: formData.cluster_group,
        is_active: formData.is_active,
        cities: citiesArray
      };

      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });

      const data = await res.json();
      if (data.success) {
        toast.success(data.message);
        router.push('/admin/cities');
      } else {
        toast.error(data.message);
      }
    } catch (error) {
      toast.error(isEdit ? 'Failed to update city' : 'Failed to save city');
    } finally {
      setSubmitting(false);
    }
  };

  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
            City Name <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            required
            placeholder="e.g., Brampton"
            className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-slate-700 dark:text-white"
            value={formData.name}
            onChange={(e) => setFormData({...formData, name: e.target.value})}
          />
        </div>
        
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
            Cluster Group <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            required
            placeholder="e.g., Greater Toronto Area"
            className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-slate-700 dark:text-white"
            value={formData.cluster_group}
            onChange={(e) => setFormData({...formData, cluster_group: e.target.value})}
          />
          <p className="text-xs text-gray-500 mt-1">Group name used to organize areas in the provider app.</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
            Mapped Cities
          </label>
          <textarea
            placeholder="e.g., Surrey, Langley, White Rock"
            rows={3}
            className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-slate-700 dark:text-white"
            value={formData.citiesInput}
            onChange={(e) => setFormData({...formData, citiesInput: e.target.value})}
          />
          <p className="text-xs text-gray-500 mt-1">Comma-separated list of exact city names that belong to this service area.</p>
        </div>

        <div className="flex items-center">
          <input
            type="checkbox"
            id="isActive"
            checked={formData.is_active}
            onChange={(e) => setFormData({...formData, is_active: e.target.checked})}
            className="w-4 h-4 text-teal-600 border-gray-300 rounded focus:ring-teal-500"
          />
          <label htmlFor="isActive" className="ml-2 text-sm text-gray-700 dark:text-slate-300">
            Active
          </label>
        </div>

        <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
          <button
            type="button"
            onClick={() => router.push('/admin/cities')}
            className="px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 rounded-lg transition"
            disabled={submitting}
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={submitting}
            className="px-4 py-2 bg-teal-600 text-white hover:bg-teal-700 rounded-lg transition disabled:opacity-50"
          >
            {submitting ? 'Saving...' : (isEdit ? 'Update City' : 'Add City')}
          </button>
        </div>
      </form>
    </div>
  );
}
